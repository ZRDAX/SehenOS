...
# Carregamento dos dados do Redis
def load_data_from_redis(r):
    keys = r.keys("packet:*")
    data = []
    for key in keys:
        packet_data = json.loads(r.get(key))
        data.append(packet_data)
        # Remover a chave do Redis após carregar os dados
        r.delete(key)
    
    # Converte os dados para DataFrame se não estiver vazio
    if data:
        df = pd.DataFrame(data)
    else:
        df = pd.DataFrame(columns=["src_ip", "dst_ip", "src_hostname", "dst_hostname", "src_mac", "dst_mac"])
    return df
...
# Função principal para executar o pipeline de ML em intervalos
def main():
    engine = connect_db()
    r = connect_redis()
    ...
    if engine and r:
        while True:
            # Carregar os dados do Redis
            df = load_data_from_redis(r)
            if df.empty:
                print("Nenhum novo dado encontrado no Redis. Aguardando...")
                time.sleep(5)  # Espera antes da próxima verificação
                continue

            # Pré-processamento e detecção de anomalias
            ...
            # Pausar antes de próxima execução para evitar sobrecarga
            time.sleep(10)
